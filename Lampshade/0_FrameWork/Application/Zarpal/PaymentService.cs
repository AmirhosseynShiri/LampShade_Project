...
namespace _0_FrameWork.Application.Zarpal
{
    ...
    {
        public PaymentStatus CreatePaymentRequest(string merchantId,
            int amount, string description, string callbackUrl,
            ref string redirectUrl, string userEmail, string userMobile)
        {
            var payment = new ZarinpalSandbox.Payment(amount);
            var res = payment.PaymentRequest(description, callbackUrl,
